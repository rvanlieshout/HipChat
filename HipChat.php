...
class HipChatPlugin extends MantisPlugin {
    ...
    function bug_report_update($event, $bug, $bug_id) {
        ...
        if ($event === 'EVENT_REPORT_BUG') {
            $msg = current_user_get_field( 'realname' ) . " added: <a href='$url'>" . ($this->has_specific_room($project) ? "" : "$project - ") . "$summary</a>";
        } else {
            $msg = current_user_get_field( 'realname' ) . " updated: <a href='$url'>" . ($this->has_specific_room($project) ? "" : "$project - ") . "$summary</a> (" . get_enum_element( 'status', $bug->status ) . ")";
        }
    ...
    function bug_deleted($event, $bug_id) {
        ...
        $reporter = '@' . user_get_name(auth_get_current_user_id());
        $summary = HipChatPlugin::clean_summary(bug_format_summary($bug_id, SUMMARY_FIELD));
        $msg = current_user_get_field( 'realname' ) . " deleted: " . ($this->has_specific_room($project) ? "" : "$project - ") . "$summary ";
    ...
    function bugnote_add_edit($event, $bug_id, $bugnote_id) {
        ...
        if ($event === 'EVENT_BUGNOTE_ADD') {
            $msg = current_user_get_field( 'realname' ) . " added note: <a href='$url'>" . ($this->has_specific_room($project) ? "" : "$project - ") . "$summary</a>";
        } else {
            $msg = current_user_get_field( 'realname' ) . " updated note: <a href='$url'>" . ($this->has_specific_room($project) ? "" : "$project - ") . "$summary</a>";
        }
    ...
    function bugnote_deleted($event, $bug_id, $bugnote_id) {
        ...
        $summary = HipChatPlugin::clean_summary(bug_format_summary($bug_id, SUMMARY_FIELD));
        $reporter = '@' . user_get_name(auth_get_current_user_id());
        $msg = current_user_get_field( 'realname' ) . " deleted note: <a href='$url'>" . ($this->has_specific_room($project) ? "" : "$project - ") . "$summary</a>";
